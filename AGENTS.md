...
## 架构硬约束 (2026-07-06 锁定, 不可改)

> 6 路 2K 拼接要求 30 FPS 端到端, 整个 pipeline 必须满足:

| 层 | 必须用 | 不接受 |
|---|---|---|
| **解码** | **gstreamer1.0-rockchip1 mppvideodec** (vendor SDK 标准组件, `dma-feature=true` 走 mpp_service 老接口) | 软件解码器 (mpeg4 sw 等); FFmpeg rkmpp wrapper (vendor 不维护) |
| **帧缓冲** | DMA-BUF backed GstBuffer (appsink 拿 `memory:DMABuf` → `gst_dmabuf_memory_get_fd` 拿 fd) | memcpy 到系统内存 (YUV420P / RGBA) |
| **RGA 路径** | RGA2 HW 裁剪/旋转/羽化 | CPU 端的 OpenCV 仿射 (warpAffine) |
| **OpenCL** | Mali GPU 接缝羽化 (clImportMemoryARM) | CPU 端 alpha blend |

**为什么放弃 FFmpeg rkmpp 走 gstreamer mppvideodec (2026-07-06 vendor 反馈)**:

- vendor 明确: "FFmpeg 存在版权, 不做支持"
- vendor 明确: kmpp 内核模块还在开发, **目前只有编码器部分, 解码器未做**
- 实测: ffmpeg 6.1 rkmpp + jellyfin-mpp 1.3.9 + 板子 vendor SDK librockchip-mpp 1.5.0 → 0 帧 (mpp_service 老接口 ABI 不兼容, strace 验证)
- 实测: 板子 SDK 自带的 `librockchip-mpp 1.5.0` 走 mpp_service 老接口, `mpi_dec_test / mpi_enc_test` 编解码 OK
- 实测: `gstreamer1.0-rockchip1 1.14-4` 包含 `mppvideodec` (H.264/HEVC/VP8/VP9), `dma-feature=true` 走 DMA-BUF 输出, **vendor 官方推荐路径**
- 实测: 一次性 `ffmpeg -c:v h264_rkmpp` 转码 MPEG-4 → H.264, mppvideodec 解出 162 帧/2.5s (7.7x 实时)

**禁止任何 sw fallback** (例如 "临时软解跑一下看效果"):

- appsink 收到非 DMA-BUF 帧 (sw path) 在 `gst_mpp_decoder.cc:184-194` 直接 reject + 报错, 不会进入 stitch 路径
- appsink caps 限定 `video/x-raw(memory:DMABuf),format=NV12` (pipeline desc 里硬编码)
- 任何"暂时 sw 解码跑一下看效果"的提案都不应合入

**当前状态** (2026-07-07): gst_mpp_decoder 写完 + sensor_data_interface 切换 + CMakeLists 加 gstreamer-1.0 依赖 + **板上 cmake/make 验证通过 + 6 路 pipeline 全部 100+ fps @ 100% DMA-BUF, 2×3 布局 (panorama 4800×4080) 启动成功**.

**数据集 faststart 处理** (2026-07-07): 原 `datasets/2k-test-h264/*.mp4` 缺 faststart moov atom, qtdemux 报 `Internal data stream error -5`. **修复**: `ffmpeg -i in.mp4 -c copy -movflags +faststart out.mp4 -y` (不要重编码 — `h264_rkmpp` 编码器现在 segfault; `-c copy` 仅移动 moov 位置, 220x 速度). 6 路全部 faststart 后, moov 在 offset 32 (ftyp 之后), qtdemux 顺利读.

**VPU/IOMMU 坑** (2026-07-07): `image-stitching` 跑超时 (`timeout 15`) 但没优雅退出 → DMA-BUF fd 累计 (单进程 56644 个) → IOMMU 虚拟地址耗尽 (`out of I/O virtual memory -28`) → 后续 mppvideodec 0 帧, `mpi_dec_test` segfault. **修复**: `ps aux | grep image-stitch; kill -9 <pid>` 后再跑. 内存从 2.6G free 释放到 7.1G. **预防**: 加 `--max-frame` 上限或 SIGTERM handler.
...
| 阶段 | 状态 | 备注 |
|---|---|---|
| 1. 板子与镜像可用性测试 | ✅ 板端工具链 OK | [§ 1.3 已完成 6 路 sensor 节点](docs/DEVELOPMENT_PLAN.md#13-6-路-mipi-拓扑验证-核心--已完成-dts-层), 自动化测试见 [post_flash_test.sh](tools/post_flash_test.sh), **gstreamer1.0-rockchip1 1.14-4 装好** (vendor SDK 标准组件, mppvideodec 走 mpp_service) |
| 2. 6 路 CameraSource 配置 (yaml) | ✅ 编译通过 + 6 路 fallback 跑通 | yaml 缩进修对 + try/catch 防解析崩, dataset fallback 6 文件 `t50-t53 + t40 + t41` H.264 (`datasets/2k-test-h264/`) 已工作, 待真机 mppvideodec DMA-BUF 验证 |
...
- 依赖：OpenCV ≥ 4.5, gstreamer-1.0 + gstreamer1.0-rockchip1 (mppvideodec), OpenCL, EGL, GLESv2, GBM, librga, libdrm, SDL2
  - FFmpeg 仍保留链接 (作为编码工具, 把 mpeg4 dataset 一次性转 h264 用; 解码路径已不走)
...
| `src/sensor_data_interface.cc` | 每路相机一个解码/采集线程，队列帧供应 (v2.4: gstreamer mppvideodec, appsink pull) |
| `src/gst_mpp_decoder.cc` | v2.4 新增: gstreamer pipeline (filesrc → qtdemux → h264parse → mppvideodec dma-feature=true → appsink), 输出 NV12 DMA-BUF fd |
...
**Pipeline 细节**: gstreamer1.0-rockchip1 mppvideodec (H.264/HEVC) → DMABuf-backed GstBuffer (appsink pull) → DMA-BUF fd → RGA crop/rotate (and optionally GLES warp) → OpenCL seam feathering → DRM output buffer.
