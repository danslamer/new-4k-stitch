...
根据最新的日志，程序并没有在后续拼接的时候对图片进行剪裁再拼接，还是直接拼接了，我需要的效果应该是通过检测roi感兴趣区域来确定每帧保留的位置，然后将4帧的感兴趣区拼接起来，每帧的ROI区域应该根据重叠的位置和相对位移来确定，也就是说需要在第一路和第二路直接确定重叠区域，通过重叠区域确定这两路的ROI高度，各自的ROI宽度应该是整帧图片的宽度减去重叠区域宽度的一半。第二路再与第三路算一次ROI，第三路再和第四路算一次ROI，这样就能确定了4路的ROI区域，然后在后续取到硬件帧之后将4路硬件帧根据ROI区域剪裁再拼接起来。现在我允许你在程序初始化的时候通过下载帧到cpu然后使用opencv检测ROI感兴趣区，将这个opencv检测出来的感兴趣区坐标保存下来在后续的拼接任务中复用。
