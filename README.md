...
版本库：从 C 回退到 B。
暂存区：内容被清空，与 B 版本保持一致。
工作区：保持不变。你在 C 版本所做的修改（hello.py 中新增的那一行）会依然存在于你的文件中，但这些修改会处于“未暂存”状态。
...
第一阶段只 remap
第二阶段做 blend/compose
中间插入可开关的 cv::ocl::finish()

二阶段 GPU profiling 实验版已经接好了。
现在主循环改成了两段执行：
先逐路 RemapImage()，然后做一次 cv::ocl::finish()；
再逐路 BlendComposeImage()，再做一次 cv::ocl::finish()。
对应代码在 src/app.cc 和 src/image_stitcher.cc。
remap_stage：主机侧逐路提交 remap 的时间
remap_finish：第一阶段显式 OpenCL finish 等待时间
compose_stage：blend + compose 提交时间
compose_finish：第二阶段显式 OpenCL finish 等待时间
...
优先尝试把 blend/compose 改回 CPU
因为现在 compose_finish 很重，说明 GPU 上做最后融合未必划算。
非常值得做一个实验版：
remap 继续走 GPU/UMat
在 remap 后把结果拉回 Mat
blend + compose 在 CPU 上做
现在我对瓶颈的判断是：

remap 本身始终很轻，1 ~ 1.5 ms
remap_finish 仍然大，60 ~ 138 ms
CPU blend/compose 也很重，90 ~ 120 ms
所以真正的问题不是“算子代码几行写法”，而是：
GPU remap 完成等待
融合这一步无论放 GPU 还是 CPU 都不便宜


我准备把权重图改成预计算的单通道 float，然后把每帧融合从三段 multiply/add 改成一段 blendLinear。这样既能减少 kernel 链长度，也能去掉每帧的 1/255 缩放负担。

减少 kernel 链是有效的
它把原来大量“记在 compose/compose_finish 上的成本”部分前移并显式暴露成了 blend 阶段时间。
...
当前工程每帧日志量非常大，而且主输出被 tee 到日志文件，额外增加了 I/O 开销。逐帧大量打印在 src/app.cc，tee 到文件在 src/app.cc。这通常不是 80 掉 3 的主因，但会继续拉低吞吐，尤其在板端存储慢的时候。
...
采集线程不是当前瓶颈。你日志里 fetch 只有 0.1~0.2ms，说明 src/sensor_data_interface.cc 这套条件变量队列并没有把 FPS 拉到 3。相反，原版 assets 用的是 sleep_for(10ms) 轮询，在 assets/sensor_data_interface.cc，这反而更粗糙。
...
先关闭逐帧保存：STITCH_SAVE_RESULTS=0。
暂时去掉 src/app.cc 的 cv::ocl::finish()，再和原版按同一口径测一次。

cv::ocl::finish() 已从 src/app.cc 这一轮主循环统计路径里去掉，FPS 口径也改成和原版一致：现在会输出 assets_style_fps，也就是只统计 fetch + gpu_warp_dispatch，不把强制同步算进去


计口径被 OpenCL 异步执行和采集线程阻塞彻底带偏了。
开启保存的日志 run_20260326_224245.log 里：
fetch 基本只有 0.1ms，比如 run_20260326_224245.log 末尾这些帧是 0.12~0.21ms
gpu_warp 也经常很小，比如 2~20ms
所以 assets_style_fps 会被算成 80、100、400+
但同一帧的 post 是 1500ms+，actual_fps 只有 0.60~0.65

关闭保存的日志 run_20260326_225555.log 里：
post 几乎是 0ms
但 fetch 变成了 130~300ms，比如 run_20260326_225555.log 的 Frame 144/147/150
于是 assets_style_fps 和 actual_fps 都掉到 3~6 FPS
...
主线程真实拼接 FPS
端到端 FPS
decode_fps、stitch_fps、end_to_end_fps

新增的 compute_only_fps 定义是：
从 fetch_done 到本帧结束 t3
也就是忽略解码/取帧等待时间，只统计主线程拿到完整帧之后的计算链路
当前包含 gpu_warp + post
...
decode_fps 基本一样
stitch_fps：cylindrical 快约 10.8%
end_to_end_fps：cylindrical 快约 14.2%

结论先说：这更像是投影模型参数的问题，不是相机标定参数本身的问题。
你现在这张图里“每一张都像被卷到柱面上”的现象，和 cylindrical/spherical 这类 warper 的视觉特征完全一致。
如果是 camchain 标定参数错了，通常会表现为：
“投影方式导致的全局几何变形”真正决定这种“柱面感”的，是 warp_type。
